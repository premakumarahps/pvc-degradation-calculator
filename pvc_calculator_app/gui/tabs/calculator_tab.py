...
Calculates the minimum required Tinuvin 770 concentration to meet targeted outdoor lifetime.
"""
...
class CalculatorTab(QWidget):
    ...
    def init_ui(self):
        ...
        lbl_lifetime.setStyleSheet("font-weight: bold; color: #f1f5f9;")
        self.input_lifetime = QLineEdit("1500")
        self.input_lifetime.setPlaceholderText("e.g. 1500 hours")
        ...
        lbl_lifetime_hint.setStyleSheet("color: #64748b; font-size: 11px;")
        grid.addWidget(lbl_lifetime_hint, 0, 2)

        # Max Dosage Input
        lbl_max_stab = QLabel("Max Stabilizer Bound (wt.%):")
        lbl_max_stab.setStyleSheet("font-weight: bold; color: #f1f5f9;")
        self.input_max_stab = QLineEdit(str(DEFAULT_MAX_STABILIZER_CONC * STABILIZER_WT_MULTIPLIER))
        self.input_max_stab.setPlaceholderText("default 0.8 wt.%")
        grid.addWidget(lbl_max_stab, 1, 0)
        grid.addWidget(self.input_max_stab, 1, 1)

        lbl_max_hint = QLabel("(Industrial standard limit typically 0.2 - 1.0 wt.%)")
        lbl_max_hint.setStyleSheet("color: #64748b; font-size: 11px;")
        grid.addWidget(lbl_max_hint, 1, 2)

        # Target PVC Retention Input
        lbl_target = QLabel("Target PVC Retention (%):")
        lbl_target.setStyleSheet("font-weight: bold; color: #f1f5f9;")
        self.input_target = QLineEdit(str(int(DEFAULT_TARGET_PVC_RETENTION * 100)))
        self.input_target.setPlaceholderText("default 80%")
        grid.addWidget(lbl_target, 2, 0)
        grid.addWidget(self.input_target, 2, 1)

        lbl_target_hint = QLabel("(Threshold before severe embrittlement & yellowing)")
        lbl_target_hint.setStyleSheet("color: #64748b; font-size: 11px;")
        grid.addWidget(lbl_target_hint, 2, 2)

        # Presets Buttons Row
        preset_layout = QHBoxLayout()
        preset_label = QLabel("Quick Presets:")
        preset_label.setStyleSheet("color: #94a3b8; font-weight: 600;")
        ...
        # Detailed Chemical Guidance
        self.txt_details = QLabel(
            "<b>Chemical Mechanism Rationale:</b><br>"
            "Tinuvin 770 acts via the regenerative Denisov cycle. Under UV exposure, sterically hindered amine "
            "groups (>NH) intercept photo-generated peroxy radicals (ROO*), converting into nitroxyl radicals (>NO*). "
            "These rapidly scavenge polymer alkyl radicals (R*) to terminate destructive auto-oxidation chain scissions."
        )
        self.txt_details.setStyleSheet("color: #94a3b8; line-height: 140%; padding: 6px;")
    ...
    def create_metric_widget(self, title: str, default_val: str) -> QWidget:
        ...
        lbl_t = QLabel(title)
        lbl_t.setStyleSheet("font-size: 11px; color: #94a3b8; font-weight: 600;")
        lbl_v = QLabel(default_val)
        lbl_v.setStyleSheet("font-size: 17px; color: #38bdf8; font-weight: 700;")
        lbl_v.setObjectName("valueLabel")
        box_layout.addWidget(lbl_t)
        box_layout.addWidget(lbl_v)
        return box

    def set_metric_value(self, metric_widget: QWidget, value: str, color: str = "#38bdf8"):
        lbl = metric_widget.findChild(QLabel, "valueLabel")
        if lbl:
            lbl.setText(value)
            lbl.setStyleSheet(f"font-size: 17px; color: {color}; font-weight: 700;")
    ...
    def run_calculation(self):
        try:
            ...
            self.set_metric_value(self.metric_compute, f"{elapsed_ms:.1f} ms")
            ...
            if result.status == OptimizationStatus.SAFE_WITHOUT_STABILIZER:
                ...
                """)
                self.set_metric_value(self.metric_wt, "0.00 wt.%", "#10b981")
                self.set_metric_value(self.metric_dosage, "0.0 g / kg", "#10b981")
                self.set_metric_value(self.metric_retention, f"{result.final_pvc_retention:.1f}%", "#10b981")
                ...
                """)
                self.set_metric_value(self.metric_wt, f"{rec_wt:.4f} wt.%", "#38bdf8")
                self.set_metric_value(self.metric_dosage, f"{g_per_kg:.2f} g / kg", "#38bdf8")
                self.set_metric_value(self.metric_retention, f"{result.final_pvc_retention:.1f}%", "#38bdf8")
                ...
                """)
                self.set_metric_value(self.metric_wt, "Exceeded", "#f59e0b")
                self.set_metric_value(self.metric_dosage, "N/A", "#f59e0b")
                self.set_metric_value(self.metric_retention, "< 80.0%", "#f59e0b")
